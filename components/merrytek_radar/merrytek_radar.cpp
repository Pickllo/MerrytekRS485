...
void MerrytekRadar::loop() {
  ...
  // Process frames
  while (this->rx_buffer_.size() >= 6) {  // Minimum frame size
    if (this->rx_buffer_[0] != FRAME_HEADER) {
      this->rx_buffer_.erase(this->rx_buffer_.begin());
      continue;
    }

    if (this->rx_buffer_.size() < 4) break; // Not enough data yet

    uint8_t len = this->rx_buffer_[3]; // LEN = FUNC + DATA
    uint8_t total_frame_len = 4 + len + 1; // header+ID+LEN + (FUNC+DATA) + CRC
    ...
    if (received_crc == calculated_crc) {
      ...
      ESP_LOGW(TAG, "CRC Failed! recv=0x%02X calc=0x%02X", received_crc, calculated_crc);
...
void MerrytekRadar::handle_frame(const std::vector<uint8_t> &frame) {
  // Frame: [HEAD, ID_H, ID_L, LEN, FUNC, DATA..., CRC]
  uint8_t func = frame[4];
  uint8_t data_len = frame[3] - 1; // LEN includes FUNC, so subtract 1 for DATA only
  const uint8_t *data = frame.data() + 5;

  ESP_LOGD(TAG, "Frame received: FUNC=0x%02X, data_len=%d", func, data_len);
  ...
  switch (func) {
    ...
    case FUNC_HOLD_TIME: {
      auto it_num = this->numbers_.find(func);
    ...
    case FUNC_PRESENCE_DETECTION_ENABLE: {
      auto it_sw = this->switches_.find(func);
    ...
    case FUNC_NEAR_ZONE_SHIELDING: {
      auto it_sel = this->selects_.find(func);
...
void MerrytekRadar::send_command(uint8_t function, const std::vector<uint8_t> &data) {
  uint8_t len = 1 + data.size(); // FUNC + DATA
  std::vector<uint8_t> frame;
  frame.reserve(4 + len + 1);
...
void MerrytekRadar::register_configurable_number(MerrytekNumber *num, uint8_t fc) {
  this->numbers_[fc] = num;
  num->set_parent(this);
  num->set_function_code(fc);
}
void MerrytekRadar::register_configurable_switch(MerrytekSwitch *sw, uint8_t fc) {
  this->switches_[fc] = sw;
  sw->set_parent(this);
  sw->set_function_code(fc);
}
void MerrytekRadar::register_configurable_select(MerrytekSelect *sel, uint8_t fc) {
  this->selects_[fc] = sel;
  sel->set_parent(this);
  sel->set_function_code(fc);
}
void MerrytekRadar::register_configurable_button(MerrytekButton *btn, uint8_t fc, const std::vector<uint8_t>& data) {
  this->buttons_[fc] = btn;
  btn->set_parent(this);
  btn->set_function_code(fc);
  btn->set_data(data);
}
...
void MerrytekButton::press_action() {
  this->parent_->send_command(this->function_code_, this->data_);
}
